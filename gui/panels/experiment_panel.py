...
# ── Experiment registry ────────────────────────────────────────────────────────
# Maps category → list of (display_name, module.ClassName)
EXPERIMENT_REGISTRY = {
...
# Default parameters shown for every experiment
_COMMON_PARAMS = {
    "py_avg":      ("int",   10,    1,    500),
...
class ExperimentPanel(QWidget):
    ...
    run_requested  = Signal(str, dict)   # (class_path, params)
    ...
    def _build_ui(self):
        ...
        self.expt_combo.currentTextChanged.connect(self._on_experiment)
        ...
        params_grp_lay = QVBoxLayout(params_grp)
        params_grp_lay.addWidget(scroll)
