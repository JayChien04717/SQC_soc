...
class BaseProgram(AveragerProgramV2):
    ...
    def setup_resonator(self, cfg, prefix="ge"):
        ...
        self.add_gauss(
            ch=res_ch, name="readout",
            sigma=cfg["res_sigma"], length=5 * cfg["res_sigma"],
            even_length=True,
        )
        self.add_pulse(
            ch=res_ch, name="res_pulse", ro_ch=ro_ch,
            style="flat_top", envelope="readout",
    ...
    def setup_qb_pulse(self, cfg, prefix='ge', pulse_type=None, shape='gauss',
                       name='qb_pulse', phase=None, gain_key=None, gain_override=None,
                       ch=None, length_mult=5):
        """
        ...
        if pulse_type in ('arb', 'flat_top'):
            ...
            if env_key not in self._added_envs:
                ...
                if shape in ('gauss', 'gaussian'):
                    self.add_gauss(
                        ch=ch, name=env_name, sigma=sigma,
                        length=sigma * length_mult, even_length=True,
                    )
        ...
        if pulse_type == 'const':
            ...
            self.add_pulse(
                ch=ch, name=name, style="arb", envelope=env_name,
                freq=freq, phase=phase, gain=gain,
            )
        elif pulse_type == 'flat_top':
            self.add_pulse(
                ch=ch, name=name, style="flat_top", envelope=env_name,
                freq=freq, phase=phase, gain=gain,
    ...
    def setup_standard_gates(self, cfg, prefix='ge', pulse_type=None, shape='gauss'):
        ...
        for gate_name, gate_phase, gain_key in gates:
            self.setup_qb_pulse(
                cfg, prefix=prefix, pulse_type=pulse_type, shape=shape,
    ...
    def apply_cool(self, cfg, style="flat_top"):
        ...
        for i in [1, 2]:
            ...
            if style == "flat_top":
                ...
                self.add_gauss(
                    ch=ch, name=env_name,
                    sigma=cfg["res_sigma"], length=cfg["res_sigma"] * 5,
                    even_length=True,
                )
                self.add_pulse(
                    ch=ch, name=f"cool_pulse{i}", envelope=env_name,
                    style="flat_top", length=cfg["cool_length"],
                    freq=cfg[f"cool_freq_{i}"], phase=0, gain=cfg[f"cool_gain_{i}"],
                )
            else:  # const
                self.add_pulse(
                    ch=ch, name=f"cool_pulse{i}", style="const",
