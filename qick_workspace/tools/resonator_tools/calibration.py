...
class calibration(object):
    ...
    def baseline_func_amp(
        ...
        return interp1d(
            f_data,
            self._baseline_als(np.absolute(z_data), lam, p, niter=niter),
            kind="cubic",
        )
    ...
    def baseline_func_phase(
        ...
        return interp1d(
            f_data,
            self._baseline_als(np.angle(z_data), lam, p, niter=niter),
            kind="cubic",
        )
    ...
    def GUIbaselinefit(self) -> None:
        ...
        self.__p = 0.9
        niter = 10
        self.__baseline = self._baseline_als(
            np.absolute(self.z_data_raw), self.__lam, self.__p, niter=niter
        )
        ...
        sSmooth = Slider(
            ...
            0.1,
            10.0,
            valinit=np.log10(self.__lam),
            valfmt="1E%f",
        )
        sAsym = Slider(
            axAsym, "Asymmetry", 1e-4, 0.99999, valinit=self.__p, valfmt="%f"
        )
        sbcorr = Slider(axbcorr, "vertical shift", 0.7, 1.1, valinit=1.0)

        def update(val):
            self.__lam = 10**sSmooth.val
            self.__p = sAsym.val
            self.__baseline = sbcorr.val * self._baseline_als(
                np.absolute(self.z_data_raw), self.__lam, self.__p, niter=niter
            )
