...
class LoopbackProgram(BaseProgram):
    ...
    def _body(self, cfg):
        ...
        if do_check_e and not self.has_qb_pulse:
            print("Warning: check_e is True but 'ge' pulse setup failed. Skipping 'ge' pulse.")
        if do_check_f and not self.has_qb_pulse_ef:
            print("Warning: check_f is True but 'ef' pulse setup failed. Skipping 'ef' pulse.")
