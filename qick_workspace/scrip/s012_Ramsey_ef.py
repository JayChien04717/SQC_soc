...
class RamseyProgram(AveragerProgramV2):
    def _initialize(self, cfg):
        ...
        if cfg["pulse_type"] == "arb":
            ...
            self.add_pulse(
                ...
                envelope="ramp_ef",
                freq=cfg["qb_freq_ef"],
                phase=cfg["qb_phase_ef"] + cfg["wait_time"] * 360 * cfg["ramsey_freq"],
                gain=cfg["pi2_gain_ef"],
            )
        elif cfg["pulse_type"] == "flat_top":
            self.add_pulse(
    ...
    def _body(self, cfg):
        ...
        self.pulse(ch=self.cfg["qb_ch_ef"], name="qb_pulse1", t=0)
        self.delay_auto(cfg["wait_time"] + 0.01, tag="wait")
        self.pulse(ch=self.cfg["qb_ch_ef"], name="qb_pulse2", t=0)
...
class Ramsey_ef:
    ...
    def plot(self):
        plt.plot(self.delay_times, np.abs(self.iqdata))
    ...
    def liveplot(self, py_avg):
        ...
        if self.cfg["ramsey_freq"] != 0:
            ...
            fig.suptitle(
                f"T2 Ramsey = {self.fit_params[3]:.2f}$\mu s, detune = {self.fit_params[1]:.5f}MHz \pm {(error[1]) * 1e3:.3f}kHz$",
                fontsize=15,
            )
            ...
            fig.suptitle(
                f"T2 Ramsey = {self.fit_params[2]:.2f}$\mu s$",
                fontsize=15,
            )
        fig.tight_layout()

    def saveLabber(self, qb_idx, yoko_value=None):
        expt_name = "s012_Ramsey_ef" + f"_{qb_idx}"
        ...
        hdf5_generator(
            ...
            comment=(f"T2 Ramsey = {self.fit_params[3]:.2f} us\n{dict_val}"),
            tag="Ramsey",
        )
