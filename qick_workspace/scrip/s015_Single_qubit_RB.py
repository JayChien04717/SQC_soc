"""
s015 — Single Qubit Randomized Benchmarking
============================================
Standard RB and Interleaved RB (IRB) for a single qubit using the
QICK BaseProgram framework.

Gate sequences are generated by ``RB_generator.single_qb_rb`` and
executed via ``RBProgram`` which unrolls the gate list at compile time.
"""
...
class RBProgram(BaseProgram):
    """QICK program that unrolls a gate sequence at compile time."""

    def _initialize(self, cfg):
        """Set up resonator, qubit generator, and standard gates."""
    ...
    def _body(self, cfg):
        """Apply optional cooling, unrolled gate sequence, then measure."""
...
class RandomizedBenchmarking:
    """
    Single-qubit Randomized Benchmarking (Standard and Interleaved).

    Instantiate with a config dict after ``BaseExperiment.setup()`` has been
    called.  Call ``run()`` to acquire data, then ``plot()`` to fit and
    ...
    def run(
        ...
        interleaved_gate : str or None, optional
            Gate name for IRB (must be in ``INTERLEAVE_GATES``), or ``None``
            for standard RB.
